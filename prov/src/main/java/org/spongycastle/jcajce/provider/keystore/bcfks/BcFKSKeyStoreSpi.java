...
import org.spongycastle.asn1.bc.SignatureCheck;
import org.spongycastle.internal.asn1.cms.CCMParameters;
...
import org.spongycastle.jcajce.BCFKSStoreParameter;
import org.spongycastle.jcajce.BCLoadStoreParameter;
import org.spongycastle.jcajce.provider.keystore.util.AdaptingKeyStoreSpi;
import org.spongycastle.jcajce.provider.keystore.util.ParameterUtil;
    ...
    extends KeyStoreSpi
{
        ...
        else if (parameter instanceof BCFKSLoadStoreParameter)
        {
            ...
            if (bcParam.getStoreSignatureKey() != null)
            {
                ...
                try
                {
                    ...
                    if (certs != null)
                    {
                        org.spongycastle.asn1.x509.Certificate[] certificates = new org.spongycastle.asn1.x509.Certificate[certs.length];
                        for (int i = 0; i != certificates.length; i++)
                        {
                            certificates[i] = org.spongycastle.asn1.x509.Certificate.getInstance(certs[i].getEncoded());
                        }
        ...
        throws IOException, NoSuchAlgorithmException, CertificateException
    {
        ...
        else if (integrityCheck.getType() == ObjectStoreIntegrityCheck.SIG_CHECK)
        {
            ...
            try
            {
                org.spongycastle.asn1.x509.Certificate[] certificates = sigCheck.getCertificates();
                if (validator != null)
                {
